...
class BedSensor(object):
    ...
    def synchronization(signal):
        """
        Method for the synchronization between the beaglebone and the bedsensor when the bedsensor send its first sample.
        """
        ...
        if match:
            ...
            samples = signal.split("$") # We split the signal into samples (debufferization)

            init_sample = samples[1]
            init_sample_bits = init_sample.split(",") # We split the first sample of the buffer.

            if self.ID != 0:
                if self.ID != int(init_sample_bits[x], 16):
                    logger.error('The synchronization is not allowed, an other bedsensor is registered on this beaglebone')
                    return False

                else:
                    logger.info('The beaglebone is resynchronizing with the bedsensor: %s' % int(init_sample_bits[x], 16))
    ...
    def matches(signal, timezone):
        ...
        if synchronization(signal) == True:
            return None, None
        ...
        if match:
            ...
            for i in range(2, nb_samples + 1):
                ...
                if sample_type == "YOP":
                    logger.error('There is a YOP sample in a wrong position in the buffer reveived')
                elif sample_type ==
