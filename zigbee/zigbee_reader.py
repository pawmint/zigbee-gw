...
def initialize_APImode():
    """
    Initialize the Xbee module in mode API/level2 => ability to get the
    emitter address and to choose the destinatory/ and to deal with the bad
...
def run(timezone):
    ...
    if i == NB_TRY:
        logger.error('The program fail to convert the Xbee module in API '
                     'mode, try to restart')
        yield {'type': 'error'}
